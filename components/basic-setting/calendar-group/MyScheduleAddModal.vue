  ...
  <section v-if="isOpen" id="addGroup" class="modal_popup" style="display: block">
              ...
              <div class="split">
                ...
                <div class="color_select">
                  <div class="SumoSelect" tabindex="0" role="button" aria-expanded="false" selected-count="1"
                    ...
                    <select v-model="state.color_select" class="multiple SumoUnder" tabindex="-1">
                      <option v-for="(color, key) in colorOptions" :key="key" :value="color">
                        {{ color }}
                      </option>
                    </select>
                    <p class="CaptionCont SelectBox multiple" title=" #1A54F5">
                      <span :style="{ backgroundColor: state.color_select }">
                        {{ state.color_select }} </span><label><i /></label>
                    </p>
                    <div v-if="colorPikerIsOpen" class="optWrapper" style="display: block">
                      <ul class="options">
                        <li v-for="(color, key) in colorOptions" :key="key" class="opt"
                          :class="{ selected: color === state.color_select }" @click="handleSetColor(color)">
    ...
    color
          ...
          <dl class="cell">
            ...
            <dd>
              <radio-components v-model="state.showSite" :options="[
    { label: '노출함', value: 'T' },
    { label: '노출하지 않음', value: 'F' },
              ...
              <div class="split left">
                ...
                <help-components name="layerAlarm">
                  ...
                  <li>
                    ...
                    건수에서 차감되는 방식이에요.<br /><a href="#" target="_blank" class="btn_link">[SMS 잔여 건수 확인하기]</a>
                  </li>
                </help-components>
              </div>
            </dt>
            <dd>
              <ul class="select_list">
                <li>
                  ...
                  <select-components :options="[{ label: '알림 없음', value: '' }]" class="selectbox" />
                </li>
                <li>
        ...
        <li>
          <div class="split left">
            <label class="label_ckeck">
              ...
              <span class="check_mark" />
              <span class="check_text">메시지
                <span class="txt_sub">(카카오알림톡 발송 후 실패 시 SMS 발송)</span></span>
            </label>
        ...
        <li>
          알림은
          <a href="#" target="_blank" class="btn_link">[메시지 사용 설정]</a>에서 설정한 방법으로 발송됩니다.
        </li>
        </help-components>
      </div>
      </li>
      ...
      <dl class="cell slt">
        ...
        <dd>
          <div ref="dropzone" class="SumoSelect" tabindex="0" role="button" aria-expanded="false" selected-count="1"
            is-selected="true">
            <select v-model="state.category_select" multiple="multiple" placeholder="선택하세요" class="multiple SumoUnder"
              tabindex="-1">
            ...
            <p class="CaptionCont SelectBox multiple" :title="state.category_select.join(', ')"
              ...
              <span>{{ state.category_select.length > 0 ? state.category_select.join(', ') : '선택하세요' }}</span>
              <label>
                <i />
              </label>
            </p>
            <div v-if="categorySelectIsOpen" class="optWrapper multiple" style="display: block">
              <ul class="options">
                <li v-for="(category, key) in categoryOptions" :key="key" class="opt" :class="{
    ...
    <div class="single_calendar">
      <div class="toggle_box">
        <a id="btnToggle" href="javascript:void(0)" class="btn_toggle" @click="showOtherIsOpenToggle()">이 그룹만 단독 캘린더로
          쇼핑몰에 보여주고 싶으신가요?<span /></a>
      ...
      <div v-if="showOtherIsOpen" class="section" style="display: block">
        ...
        <dl class="cell">
          <dt>단독 캘린더 사용여부</dt>
          <dd>
            <radio-components v-model="state.singleCalUse" :options="[
    { label: '사용함', value: 'T' },
    { label: '사용 안 함', value: 'F' },
        ...
        <dl class="cell">
          <dt>접근 권한</dt>
          <dd>
            <radio-components v-model="state.frontGrade" :options="[
    { label: '전체 허용', value: 'T' },
    { label: '회원만 허용', value: 'F' },
  ]" />
            <div id="frontGrade" class="radio_cont">
              <checkbox-components v-model="state.roles" :options="[
    { label: '전체', value: '' },
    { label: '새싹', value: '1' },
    { label: '일반', value: '2' },
    { label: 'VIP', value: '3' },
        ...
        <dl class="cell">
          <dt>기본 화면</dt>
          <dd>
            <radio-components v-model="state.frontBase" :options="[
    { label: '월', value: 'M' },
    { label: '주', value: 'W' },
    { label: '일', value: 'D' },
  ]" />
        ...
        <dl class="cell">
          <dt>한주의 시작</dt>
          <dd>
            <radio-components v-model="state.frontWeek" :options="[
    { label: '일요일', value: 'S' },
    { label: '월요일', value: 'M' },
  ]" />
          </dd>
