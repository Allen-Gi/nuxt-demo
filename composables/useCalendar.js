export const useCalendar = () => {
  ...
  const data = ref([
    ...
    },
    {
      ...
      description:
        '안녕하세요, 11번가입니다.<br><br>2023년 매 최고의 판매 찬스~! 월간 십일절이 5일간 더 새롭게 진행됩니다.<br><br>5일로 늘어난 ‘월간십일절 프로모션’ 은 11번가와 판매자가 최대 매출 달성을 위 매월 고정으로 진행하는 분담 프로모션입니다. <br><br>해당 행사는 판매자 분들이 직 신청하는 프로모션의 연간 최고 거래액을 달성할 수 있도 매월 정기적으로 신청해 주세요.',
      url: 'http://naver.com',
    },
    // calendarGroup2
    {
  ...
  const filterImportantEvents = (important = false) => {
    filteredEvents.value = important
      ? data.value.filter(
          (event) =>
            allSchedule.value.find((group) => group.group === event.group)
              .important,
        )
      : data.value
  }
   ...
   */
  const filterMyGroup = () => {
    filteredEvents.value = allSchedule.value
      .filter((group) => group.checked)
      .map((group) => data.value.filter((event) => event.group === group.group))
      .flat()
  }
  ...
  const searchMySchedule = (searchText) => {
    filteredEvents.value = data.value.filter((event) =>
